...
//~ {
    ...
    //~ auto res = executeShell(rdmdApp);
    //~ assert(res.status == 1);
    //~ // note: have to compare lines rather than the whole output due to \r != \r\n case.
    //~ assert(res.output.strip.split == helpString.strip.split);

    //~ /* Test --help. */
    //~ res = executeShell("%s --help".format(rdmdApp));
    //~ assert(res.status == 0);
    //~ assert(res.output.strip.split == helpString.strip.split);
    ...
    //~ res = executeShell("%s %s".format(rdmdApp, forceSrc));
    //~ assert(res.status == 0);
    ...
    //~ res = executeShell("%s --force %s".format(rdmdApp, forceSrc));
    //~ assert(res.status == 0);
    ...
    //~ res = executeShell("%s --force --build-only %s".format(rdmdApp, failRuntime));
    //~ assert(res.status == 0);  // only built, assert(0) not called.

    //~ res = executeShell("%s --force %s".format(rdmdApp, failRuntime));
    //~ assert(res.status == 1);  // assert(0) called, rdmd execution failed.

    //~ string failComptime = tempDir().buildPath("fail_comptime_.d");
    //~ std.file.write(failComptime, "void main() { static assert(0); }");

    //~ res = executeShell("%s --force --build-only %s".format(rdmdApp, failComptime));
    //~ assert(res.status == 1);  // building will fail for static assert(0).

    //~ res = executeShell("%s --force %s".format(rdmdApp, failComptime));
    //~ assert(res.status == 1);  // ditto.

    //~ /* Test --chatty. */
    //~ string voidMain = tempDir().buildPath("void_main_.d");
    //~ std.file.write(voidMain, "void main() { }");

    //~ res = executeShell("%s --force --chatty %s".format(rdmdApp, voidMain));
    //~ assert(res.status == 0);
    //~ assert(res.output.canFind("stat "));  // stat should be called.

    //~ /* Test --compiler. */
    //~ string compScript = tempDir().buildPath("compScript_") ~ binExt;
    //~ string compScriptSrc = tempDir().buildPath("compScriptSrc_.d");
    //~ std.file.write(compScriptSrc,
        //~ "int main(string[] args) { if (!args.length) { assert(0); } return args[1..$].length; }");

    //~ // build the app to be used with --compiler.
    //~ res = executeShell("dmd -of%s %s".format(compScript, compScriptSrc));
    //~ assert(res.status == 0);

    //~ // test it
    //~ res = executeShell("%s --force --compiler=%s %s".format(rdmdApp, compScript, voidMain));
    //~ assert(res.status >= 4);  // compiler called with: [-of, -od, -I, source_file].

    //~ /* Test --dry-run. */
    //~ res = executeShell("%s --force --dry-run %s".format(rdmdApp, failComptime));
    //~ assert(res.status == 0);  // static assert(0) not called since we did not build.
    //~ assert(res.output.canFind("stat "));  // --dry-run implies chatty, so stat is called.

    //~ /* Test --eval. */
    //~ res = executeShell("%s --force --eval=%s %s".format(rdmdApp, "writeln(`eval_works`);", failComptime));
    //~ assert(res.status == 0);
    ...
    //~ res = executeShell("dmd -c -of%s %s".format(subModObj, subModSrc));
    //~ assert(res.status == 0);
    ...
    //~ res = executeShell("%s --force --exclude=dsubpack %s".format(rdmdApp, subModUser));
    //~ assert(res.status == 1);  // building without the dependency fails

    //~ res = executeShell("%s --force --exclude=dsubpack %s %s".format(rdmdApp, subModObj, subModUser));
    //~ assert(res.status == 0);  // building with the dependency succeeds

    //~ /* Test --loop. */
    //~ string fooBarText = tempDir().buildPath("fooBarText_.txt");
    //~ std.file.write(fooBarText,
        //~ "foo\nbar\ndoo");

    //~ res = executeShell("%s --force --loop=writeln(line); < %s".format(rdmdApp, fooBarText));
    //~ assert(res.status == 0);
    ...
    //~ /+ res = executeShell("%s %s".format(rdmdApp, noMain));
    //~ assert(res.status == 1);  // main missing +/

    //~ res = executeShell("%s --main %s".format(rdmdApp, noMain));
    //~ assert(res.status == 0);  // main added

    //~ string intMain = tempDir().buildPath("int_main_.d");
    //~ std.file.write(intMain, "int main(string[] args) { return args.length; }");

    //~ res = executeShell("%s --main %s".format(rdmdApp, intMain));
    //~ assert(res.status == 1);  // duplicate main
    ...
    //~ res = executeShell("%s -I%s --makedepend %s".format(rdmdApp, packRoot, depMod));
    //~ assert(res.output.canFind("depMod_.d : "));  // simplistic check

    //~ /* Test --shebang. */
    //~ // Todo: Not sure what --shebang does
//~ }
