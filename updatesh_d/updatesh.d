...
import std.stdio;
import std.file:getcwd,tempDir,mkdirRecurse,rmdirRecurse,exists,chdir,isDir,setTimes;
import std.path:dirSeparator;
import std.datetime:Clock;
import scriptutil:universalWhich,fileExists,TempDirMode,makeTempEntry,fileIsWritable,joinPath,amIRoot;
import std.typecons:Tuple,tuple;
import std.string:indexOf,canFind,toLower,strip;
...
else version(Windows)
{
	...
	static assert(0,"updatesh currently only supports Posix or Windows");
...
// Take care of the command line arguments
	writefln("* updatesh.d - tool for updating DMD build from github\n");
	foreach(arg;args[1..$])
	{
    	if (arg[0..5]=="--tag")
    	{
...
{
	 writefln(
 	"updatesh.d - for the D programming language
	\n
